...
from GPT_GNN.conv import GeneralConv
...
class GPT_GNN(nn.Module):
    ...
    def link_loss(self, node_emb, rem_edge_list, ori_edge_list, node_dict, target_type, use_queue=True,
                  update_queue=False):
        losses = 0
        ress = []
        for source_type in rem_edge_list:
            ...
            for relation_type in rem_edge_list[source_type]:
                ...
                negative_source_ids = [self.neg_sample(source_node_ids, \
                                                       ori_edges[ori_edges[:, 0] == t_id][:, 1].tolist()) for t_id in
                                       target_ids]
    ...
    def text_loss(self, reps, texts, w2v_model, device):
        def parse_text(texts, w2v_model, device):
            idxs = []
            pad = w2v_model.wv.key_to_index['eos']  # From previous "w2v_model.wv.vocab['eos'].index"
            for text in texts:
                idx = []
                for word in ['bos'] + preprocess_string(text) + ['eos']:
                    if word in w2v_model.wv.key_to_index:
                        idx += [w2v_model.wv.key_to_index[word]]
                idxs += [idx]
            ...
            return torch.LongTensor(inp_idxs).transpose(0, 1).to(device), \
                   torch.LongTensor(out_idxs).transpose(0, 1).to(device), torch.BoolTensor(masks).transpose(0, 1).to(
                device)
...
class Matcher(nn.Module):
    ...
    def forward(self, x, ty, use_norm=True, AD_task=False):
        if AD_task:
            tx = x  # Condition added by myself to test "finetune_AD"
        else:  # The original default version
