...
from GPT_GNN.data import renamed_load, sample_subgraph, to_torch
from GPT_GNN.utils import args_print, feature_OAG, randint, ndcg_at_k
from GPT_GNN.model import GPT_GNN, Matcher, GNN, RNNModel

parser = argparse.ArgumentParser(description='Pre-training HGT on a given graph')
...
parser.add_argument('--w2v_dir', type=str, default='preprocessed/w2v_all',
...
parser.add_argument('--data_dir', type=str, default='preprocess_output/graph_5gram_key20_yake_x.pk',
                    help='The address of preprocessed graph.')
parser.add_argument('--pretrain_model_dir', type=str, default='models/model_5gram_key20_yake_x',
...
args_print(args)  # Print parameters & the corresponding values
...
# print("No. of pre_target_nodes:", pre_target_nodes.shape[0])
# print("No. of train_target_nodes:", train_target_nodes.shape[0])
...
def GPT_sample(seed, target_nodes, time_range, batch_size, feature_extractor):
    ...
    for source_type in edge_list[target_type]:
        for relation_type in edge_list[target_type][source_type]:
            ...
            if len(el) > 0:
                edge_list[target_type][source_type][relation_type] = np.concatenate(
                    (edge_list[target_type][source_type][relation_type], el))
...
def prepare_data(pool):
    ...
    for _ in np.arange(args.n_batch - 1):
        jobs.append(  # "feature_OAG" is specially designed in "utils.py"!
            pool.apply_async(GPT_sample, args=(randint(), pre_target_nodes, pre_range, args.batch_size, feature_OAG)))
    jobs.append(
        pool.apply_async(GPT_sample, args=(randint(), train_target_nodes, train_range, args.batch_size, feature_OAG)))
    return jobs
...
gnn = GNN(conv_name=args.conv_name, in_dim=len(graph.node_feature[target_type]['emb'].values[0]) + len(graph.node_feature[target_type]['abs_emb'].values[0]) + 401,
...
if args.attr_type == 'text':
    ...
    n_tokens = len(
        w2v_model.wv)  # Change from w2v_model.wv.vocab because "vocab attribute was removed from KeyedVector in Gensim 4.0.0"
    attr_decoder = RNNModel(n_word=n_tokens, ninp=gnn.n_hid, nhid=w2v_model.vector_size, nlayers=2)
    attr_decoder.from_w2v(torch.FloatTensor(w2v_model.wv.vectors))
else:
    attr_decoder = Matcher(gnn.n_hid, gnn.in_dim)

gpt_gnn = GPT_GNN(gnn=gnn, rem_edge_list=rem_edge_list, attr_decoder=attr_decoder, neg_queue_size=0, types=types,
                  neg_samp_num=args.neg_samp_num, device=device)
...
if args.scheduler == 'cycle':
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, pct_start=0.02, anneal_strategy='linear',
                                                    final_div_factor=100, max_lr=args.max_lr,
                                                    total_steps=repeat_num * args.n_batch * args.n_epoch + 1)
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    for batch in np.arange(repeat_num) + 1:
        ...
        for data, rem_edge_list, ori_edge_list, attr, (start_idx, end_idx) in train_data:
            ...
            loss = loss_link + loss_attr * args.attr_ratio  # = loss_link * (1 - args.attr_ratio) + loss_attr * args.attr_ratio
        ...
        with torch.no_grad():
            ...
            st = time.time()
            print((
                      "Epoch: %d, (%d / %d) %.1fs  LR: %.5f Train Loss: (%.3f, %.3f)  Valid Loss: (%.3f, %.3f)  NDCG: %.3f  Norm: %.3f  queue: %d") % (
                      epoch, batch, repeat_num, (st - et), optimizer.param_groups[0]['lr'],
                      np.average(train_link_losses),
                      np.average(train_attr_losses), loss_link, loss_attr, np.average(ndcgs),
